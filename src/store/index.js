...
export default new Vuex.Store({
  ...
  getters: {
    ...
    filterList(state) {
      ...
      filterList.push({
        ...
        value: Array.from(new Set(state.todoList.map(el => Number(el.userId)))),
      });
      filterList.push({
        id: "completed",
  ...
  actions: {
    async checkUsersAction({ commit }, { userName, userPhone }) {
    ...
    async addNewTaskAction( { commit }, taskData) {
      const addTodo = await fetch("https://jsonplaceholder.typicode.com/todos",{
